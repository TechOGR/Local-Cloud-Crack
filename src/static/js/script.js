...
input_file.addEventListener("change", async (e) => {
  ...
  const listFilesTransfer = new DataTransfer();
      ...
      .then((data) => {
        ...
        for (let i = 0; i < keys_name.length; i++) {
          if (values_exists[i] === false) {
            ...
            listFilesTransfer.items.add(file);
          } else {
            const index =
              input_file.files[i].name == keys_name[i]
                ? input_file.files[i]
                : false;
  ...
  DataFiles.push(listFilesTransfer);

  for (let i = 0; i < listFilesTransfer.files.length; i++) {
    console.log(`####${listFilesTransfer.files[i].name}####`);
  }
  ...
  if (objectFiles.length != 1 && objectFiles.length != 0) {
    ...
    button_element.addEventListener("click", (e) => {
      const buttons_close_item = document.querySelectorAll(
        "#container-btn-close"
      );
